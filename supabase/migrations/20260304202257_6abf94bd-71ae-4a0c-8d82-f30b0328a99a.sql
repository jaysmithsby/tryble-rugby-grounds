...
BEGIN
  ...
  -- If scores are cleared (set back to NULL), reset all prediction points to NULL
  IF (NEW.score_a IS NULL OR NEW.score_b IS NULL)
     AND (OLD.score_a IS NOT NULL AND OLD.score_b IS NOT NULL)
  THEN
    UPDATE predictions SET points_earned = NULL, updated_at = now() WHERE fixture_id = NEW.id;
    RETURN NEW;
  END IF;
